...
/// User-initiated lifecycle actions for one configured MCP server.
...
pub enum Op {
    ...
    ReloadHarnesses,
    /// Drive one MCP connection without coupling a frontend to transport/auth code.
    McpControl {
        name: String,
...
pub enum Event {
    ...
    /// Authentication lifecycle for a remote MCP server.
    McpAuthStatus {
        name: String,
        state: String,
        detail: String,
    },
    /// Browser authorization URL generated by the engine for a frontend to open.
    McpAuthorizationUrl { name: String, url: String },
...
mod tests {
    ...
    #[test]
    fn mcp_control_operations_serialize_typed_actions() {
        for (action, expected) in [
            (McpControlAction::Test, "test"),
            (McpControlAction::Authorize, "authorize"),
            (McpControlAction::Disconnect, "disconnect"),
            (McpControlAction::Reconnect, "reconnect"),
        ] {
            let op = Op::McpControl {
                name: "supabase".to_string(),
                action,
            };
            let value = serde_json::to_value(&op).unwrap();

            assert_eq!(value["op"], "mcp_control");
            assert_eq!(value["name"], "supabase");
            assert_eq!(value["action"], expected);
            ...
            assert!(matches!(
                ...
                Op::McpControl {
                    name,
                    action: decoded_action,
                } if name == "supabase" && decoded_action == action
            ));
        }
    }

    #[test]
    fn mcp_auth_events_serialize_without_secret_fields() {
        let status = Event::McpAuthStatus {
            name: "supabase".to_string(),
            state: "authorized".to_string(),
            detail: "Stored in the system credential vault".to_string(),
        };
        let status_value = serde_json::to_value(&status).unwrap();
        assert_eq!(status_value["event"], "mcp_auth_status");
        assert_eq!(status_value["name"], "supabase");
        assert_eq!(status_value["state"], "authorized");
        assert!(status_value.get("access_token").is_none());

        let authorization = Event::McpAuthorizationUrl {
            name: "supabase".to_string(),
            url: "https://api.supabase.com/v1/oauth/authorize?state=opaque".to_string(),
        };
        let authorization_value = serde_json::to_value(&authorization).unwrap();
        assert_eq!(authorization_value["event"], "mcp_authorization_url");
        assert_eq!(authorization_value["name"], "supabase");
    }
